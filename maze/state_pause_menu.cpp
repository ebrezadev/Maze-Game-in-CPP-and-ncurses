...
State_Pause_Menu::state_return_t State_Pause_Menu::stateFunction(int input)
{
	...
	if (screen_update_flag)
	{
		...
		if (current_menu == menu_t::CONTINUE)
		{
			...
			attron(A_DIM);
			mvprintw(6, (max_columns - strlen("restart")) / 2, "%s", "restart");
			mvprintw(7, (max_columns - strlen("main menu")) / 2, "%s", "main menu");
			mvprintw(8, (max_columns - strlen("exit")) / 2, "%s", "exit");
			attroff(A_DIM);
		}
		else if (current_menu == menu_t::RESTART)
		{
			mvwaddch(stdscr, 6, ((max_columns - strlen("restart")) / 2) - 2, ACS_DIAMOND);
			mvprintw(6, (max_columns - strlen("restart")) / 2, "%s", "restart");
			attron(A_DIM);
			mvprintw(5, (max_columns - strlen("continue")) / 2, "%s", "continue");
			mvprintw(7, (max_columns - strlen("main menu")) / 2, "%s", "main menu");
		...
		{
			mvwaddch(stdscr, 7, ((max_columns - strlen("main menu")) / 2) - 2, ACS_DIAMOND);
			mvprintw(7, (max_columns - strlen("main menu")) / 2, "%s", "main menu");
			attron(A_DIM);
			mvprintw(5, (max_columns - strlen("continue")) / 2, "%s", "continue");
			mvprintw(6, (max_columns - strlen("restart")) / 2, "%s", "restart");
		...
		{
			mvwaddch(stdscr, 8, ((max_columns - strlen("exit")) / 2) - 2, ACS_DIAMOND);
			mvprintw(8, (max_columns - strlen("exit")) / 2, "%s", "exit");
			attron(A_DIM);
			mvprintw(5, (max_columns - strlen("continue")) / 2, "%s", "continue");
			mvprintw(6, (max_columns - strlen("restart")) / 2, "%s", "restart");
	...
	{
	case KEY_UP:
		current_menu = (menu_t)(((int)current_menu + 3) % 4);
		screen_update_flag = true;
		break;
	case KEY_DOWN:
		current_menu = (menu_t)(((int)current_menu + 1) % 4);
		screen_update_flag = true;
		break;
	case (int)13:
		if (current_menu == menu_t::CONTINUE)
